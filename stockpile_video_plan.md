...
*   **LLM / Vision**: Google Gemini 1.5 Pro (via `google-genai` SDK)
*   **Video Engine**: `moviepy` (v2.0+) for composition, `ffmpeg` for raw processing.
*   **Audio**: ElevenLabs API (best quality) or `edge-tts` (free alternative).
*   **Stock Footage**: Pexels API (Free, high limits) / Pixabay.
*   **Subtitles**: `faster-whisper` for precise timestamping + `moviepy` TextClips.
*   **Images**: Google Imagen / Gemini generation for specific missing visuals.
